      ...
      <div class="bg-white rounded-lg shadow-lg p-8">
        ...
        <div v-if="strategy" class="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
          ...
          <div class="bg-purple-50 p-4 rounded-lg">
            <div class="text-2xl font-bold text-purple-600">{{ (stats.betCount > 0 ? (stats.wonCount / stats.betCount * 100).toFixed(1) : 0) }}%</div>
              ...
              <tbody class="bg-white divide-y divide-gray-200">
                <tr v-for="(result, index) in recentResults.slice().reverse().slice(0, 10)" :key="index">
                  ...
                  <td class="px-6 py-4 whitespace-nowrap">
                    ...
                    >
                      {{ result.won ? 'Win' : 'Loss' }} {{ result.won ? '+' : '' }}{{ formatCurrency(result.totalPayout - result.totalBet) }}
                    </span>
                  </td>
                  <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
...
import { RouletteType, SpotGenerateType, Spot, Strategy, SimulationResult, StrategyStats } from '~/types';
